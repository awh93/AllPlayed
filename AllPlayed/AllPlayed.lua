...
local col_align = {}
local pvp_columns = { 'show_arena_points', 'show_honor_points', 'show_honor_kills', 'show_conquest_points', 'show_arena_played', 'show_arena_won' }
...
if type(pc_data.last_update) ~= "number" then
	err("realm = %s, pc = %s, pc_data.last_update = %s", realm, pc, pc_data.last_update)
							...
							if self:GetOption('show_arena_played') then
								...
								col_text[col_no] = C:White(('%d'):format(played))
								col_align[col_no] = 'CENTER'
								col_no = col_no + 1
								col_text[col_no] = ''
							end

							if self:GetOption('show_arena_won') then
								local played = pc_data.arena_played or 0
								local won    = pc_data.arena_won or 0
								local pct    = played > 0 and (won / played * 100) or 0
								col_text[col_no] = C:White(('%d'):format(won)) .. C:Green((' (%.0f%%)'):format(pct))
								col_align[col_no] = 'CENTER'
								col_no = col_no + 1
								col_text[col_no] = ''
							end
	...
	if self:GetOption('show_arena_totals') and (self:GetOption('show_arena_played') or self:GetOption('show_arena_won')) then
		...
		tooltip:SetCell(line, nb_columns,
			C:White(('%d/%d'):format(total_won, total_played)) ..
			C:Green((' (%.0f%%)'):format(pct)) ..
			C:Yellow((' [2v2:%d/%d 3v3:%d/%d]'):format(total_2v2w, total_2v2p, total_3v3w, total_3v3p)),
			'RIGHT')
	end
