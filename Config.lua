...
-- Version identification management
do
	local version_string = nil

	function GetVersionString()
		if not version_string then
			-- Find the current revision number from all the files revisions
			local revision = 0
			...
			if C_AddOns and C_AddOns.GetAddOnMetadata then
				version = C_AddOns.GetAddOnMetadata(AP_display_name, "Version"):match("([^ ]+)")
			else
				version = _G.GetAddOnMetadata(AP_display_name, "Version"):match("([^ ]+)")
			end
...
local function ReturnConfigMenu()
	-- This function is called only once. Otherwise, I would recycle the table
	local config_menu = {
		[1] = {
		...
		[4] = {
			...
			menuList = {
				...
				[2] = {
					text = L["All Realms"];
					tooltipText = L["All realms will be displayed"];
					checked = 'all_realms';
				},
				[3] = {
				...
				[6] = {
					text = L["Show Gold"];
					tooltipText = L["Display the gold each character possesses"];
					checked = 'show_coins';
				},
				[7] = {
				...
				[11] = {
					...
					menuList = {
						[1] = {
							...
							tooltipText = (L["Display the %s each character possesses"]):format(L["Valor Points"]);
				...
				[12] = {
					...
					menuList = {
						[1] = {
							...
							tooltipText = (L["Display the %s each character possesses"]):format(L["Justice Points"]);
				...
				[14] = {
					...
					menuList = {
						...
						[2] = {
							...
							tooltipText = (L['Display the %s each character possesses']):format(L["Honor Points"]);
							checked = 'show_honor_points';
						},
						[3] = {
							text = (L['Display the %s each character possesses']):format(
																			   L["Conquest Points"],
																			   conquest_alliance_icon,
																			   conquest_horde_icon);
							tooltipText = (L['Display the %s each character possesses']):format(L["Conquest Points"]);
		...
		[5] = {
			...
			menuList = {
				...
				[2] = {
					...
					tooltipText = L["Use the current sort type in reverse order"];
...
do
	function DisplayConfigMenu(anchorFrame)
		-- Відкриваємо стандартне вікно налаштувань аддона, тільки якщо не в бою
		if InCombatLockdown and InCombatLockdown() then
			print("AllPlayed: Не можна відкривати налаштування під час бою!")
			return
		end
		-- Блокуємо відкриття меню, якщо активне Blizzard GameMenuFrame (уникаємо ADDON_ACTION_FORBIDDEN)
		if GameMenuFrame and GameMenuFrame:IsShown() then
			print("AllPlayed: Не можна відкривати налаштування під час відкритого Blizzard Game Menu!")
			return
		end
		if InterfaceOptionsFrame_OpenToCategory then
			InterfaceOptionsFrame_OpenToCategory(AP_display_name)
			InterfaceOptionsFrame_OpenToCategory(AP_display_name) -- двічі для коректного фокусу
		elseif SettingsPanel and SettingsPanel.OpenToCategory then
			SettingsPanel:OpenToCategory(AP_display_name)
		else
			print("AllPlayed: Unable to open config panel. Please use /allplayed або через меню аддонів.")
		end
	end
end -- do
		...
		args = {
			display = {
				type = 'group', name = L["Display"], desc = L["Specify what to display"], args = {
					...
					show_coins = {
						name      = L["Show Gold"],
						desc      = L["Display the gold each character possesses"],
						type      = 'toggle',
					...
					show_class_icon = {
						...
						desc      = L["Show the character class icon next to the name"],
						type      = 'toggle',
						get       = function() return AllPlayed:GetOption('show_class_icon') end,
						set       = function(info, v) AllPlayed:SetOption('show_class_icon',v) end,
						order     = 1.56,
					},
					show_resting_indicator = {
						name      = L["Show Resting Indicator"],
						desc      = L["Show a 'Zzz' icon when the character is resting"],
						type      = 'toggle',
						get       = function() return AllPlayed:GetOption('show_resting_indicator') end,
						set       = function(info, v) AllPlayed:SetOption('show_resting_indicator',v) end,
						order     = 1.57,
					},
					...
					show_valor_points = {
						...
						desc      = (L["Display the %s each character possesses"]):format(L["Valor Points"]),
						type      = 'toggle',
					...
					show_justice_points = {
						...
						desc      = (L["Display the %s each character possesses"]):format(L["Justice Points"]),
						type      = 'toggle',
					...
					reverse_sort = {
					  ...
					  desc      = L["Use the current sort type in reverse order"],
					  type      = 'toggle',
					...
					show_honor_points = {
						...
						desc        = (L['Display the %s each character possesses']):format(L["Honor Points"]),
						type        = 'toggle',
					...
					show_conquest_points = {
						...
						desc        = (L['Display the %s each character possesses']):format(L["Conquest Points"]),
						type        = 'toggle',
					...
					show_arena_played = {
						name        = L['Arenas Played'],
						desc        = L['Show total arena matches played per character (from Statistics panel)'],
						type        = 'toggle',
						get         = function() return AllPlayed:GetOption('show_arena_played') end,
						set         = function(info, v) AllPlayed:SetOption('show_arena_played',v) end,
						order = 11.5,
					},
					show_arena_won = {
						name        = L['Arenas Won'],
						desc        = L['Show total arena wins per character with win rate (from Statistics panel)'],
						type        = 'toggle',
						get         = function() return AllPlayed:GetOption('show_arena_won') end,
						set         = function(info, v) AllPlayed:SetOption('show_arena_won',v) end,
						order = 11.6,
					},
					show_arena_totals = {
						name        = L['Show Arena Totals'],
						desc        = L['Show combined arena totals (all chars) at the bottom of the tooltip, with 2v2/3v3 breakdown'],
						type        = 'toggle',
						get         = function() return AllPlayed:GetOption('show_arena_totals') end,
						set         = function(info, v) AllPlayed:SetOption('show_arena_totals',v) end,
						order = 11.7,
					},
				}, order = 10,
			},
			...
			delete = {
				...
				desc    = L["Erase character data permanently"],
				type    = 'group',
				args    = {
					delete_desc = {
						name = L["Erase character data permanently"],
						type = 'description',
						order = .5,
					},
					delete_desc2 = {
			...
			ui = {
				type = 'group', name = L["UI"], desc = L["Set UI options"], args = {
					...
					ldb_text_type = {
						name      = L["LDB Text Type"],
						desc      = L["Select what information to display on the LDB panel"],
						width		= "full",
						type      = 'select',
						get       = function() return AllPlayed:GetOption('ldb_text_type') end,
						set       = function(info, v) AllPlayed:SetOption('ldb_text_type',v); AllPlayed:OnDataUpdate() end,
						values    = { ["time"] = L["Time Played"], ["gold"] = L["Gold"], ["none"] = L["None"] },
						order     = .91,
					},
					show_shading = {
						name      = L["Show Shading"],
						desc      = L["Show alternating background shading in the tooltip"],
						width		= "full",
						type      = 'toggle',
						get       = function() return AllPlayed:GetOption('show_shading') end,
						set       = function(info, v) AllPlayed:SetOption('show_shading',v) end,
						order     = .92,
					},
				}, order = 30,
			},
