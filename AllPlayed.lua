...
if C_AddOns and C_AddOns.GetAddOnMetadata then
	AllPlayed_revision.toc  = C_AddOns.GetAddOnMetadata(AP_display_name, "Version"):match("%$Revision:%s(%d+)")
else
	AllPlayed_revision.toc  = _G.GetAddOnMetadata(AP_display_name, "Version"):match("%$Revision:%s(%d+)")
end

-- Backward and forward compatibility when playing Cataclysm
local GetHonorCurrency = _G.GetHonorCurrency or function()
	if C_CurrencyInfo and C_CurrencyInfo.GetCurrencyInfo then
		local info = C_CurrencyInfo.GetCurrencyInfo(392)
...
local GetConquestCurrency = _G.GetConquestCurrency or function()
	if C_CurrencyInfo and C_CurrencyInfo.GetCurrencyInfo then
		local info = C_CurrencyInfo.GetCurrencyInfo(390)
		return info and info.quantity or 0
	end
	return 0
end
local GetJusticeCurrency = _G.GetJusticeCurrency or function()
	if C_CurrencyInfo and C_CurrencyInfo.GetCurrencyInfo then
		local info = C_CurrencyInfo.GetCurrencyInfo(395)
		return info and info.quantity or 0
	end
	return 0
end
...
-- Creation of the main "object" with libraries (mixins) directly attached to the object (use self:functions)
...
local myfullname
if C_AddOns and C_AddOns.GetAddOnMetadata then
	myfullname = C_AddOns.GetAddOnMetadata(AP_display_name, "Title")
else
	myfullname = _G.GetAddOnMetadata(AP_display_name, "Title")
end
...
function AllPlayed:OnInitialize()
	...
	-- Cache for computed totals (optimization)
	self.totals_cache = {
		valid = false,
		last_compute_time = 0,
		honor_valid = false,
		honor_last_compute_time = 0,
	}
	
	-- Dirty flag for optimized updates
	self.data_dirty = false

	-- Find the max level based on WoW version
	-- Retail 12.x+ = Level 90 (The War Within and beyond)
	-- MoP Classic = Level 90 (Mists of Pandaria)
	-- Wrath Classic = Level 80
	-- BC Classic = Level 70 (The Burning Crusade)
	-- Classic Era = Level 60
	
	-- First determine max level based on interface version (most reliable for Classic variants)
	local interface_version = select(4, _G.GetBuildInfo()) or 0
	
	if interface_version >= 120000 then
		-- Retail 12.x+ (The War Within and beyond) - Level 90
		self.max_pc_level = 90
	elseif interface_version >= 50000 and interface_version < 100000 then
		-- MoP Classic 5.x - Level 90
		self.max_pc_level = 90
	elseif interface_version >= 30000 and interface_version < 50000 then
		-- Wrath Classic 3.x - Level 80
		self.max_pc_level = 80
	elseif interface_version >= 20500 and interface_version < 30000 then
		-- BC Classic 2.x - Level 70
		self.max_pc_level = 70
	elseif interface_version >= 11500 and interface_version < 20500 then
		-- Classic Era 1.x - Level 60
		self.max_pc_level = 60
	else
		-- Unknown version, try other methods
		self.max_pc_level = nil
	end
	
	-- For Retail, use the proper API if available (it's more accurate for retail)
	if C_PlayerInfo and C_PlayerInfo.GetMaxLevelForExpansionLevel and interface_version >= 100000 then
		local expansionLevel = math.min(_G.GetExpansionLevel(), _G.GetAccountExpansionLevel())
		self.max_pc_level = C_PlayerInfo.GetMaxLevelForExpansionLevel(expansionLevel)
	end
	
	-- If we still don't have a max level, try MAX_PLAYER_LEVEL as a last resort
	if not self.max_pc_level and _G.MAX_PLAYER_LEVEL and _G.MAX_PLAYER_LEVEL > 0 then
		self.max_pc_level = _G.MAX_PLAYER_LEVEL
	end
	
	-- Final fallback
	if not self.max_pc_level then
		self.max_pc_level = 90
	end
...
function AllPlayed:OnEnable()
	...
	-- Оптимізація: використовуємо більший інтервал для фонових оновлень
	local refresh_interval = self:GetOption('show_seconds') and 1 or 30
	self.timer = self:ScheduleRepeatingTimer("MyUpdate", refresh_interval)
	
	-- Викликаємо негайне оновлення при видимих змінах
...
function AllPlayed:MyUpdate(...)
	...
	-- Оптимізація: оновлюємо тільки якщо є зміни або tooltip видимий
	-- Захист від nil для версії 12.0.0
	local last_update = self.last_visible_update or 0
	local needs_update = self.data_dirty or self.tooltip or (time() - last_update > 60)
	
	if needs_update then
		self:OnDataUpdate()
                local ldb_type = self:GetOption('ldb_text_type')
                if ldb_type == "gold" then
                    AllPlayedLDB.text = FormatMoney(self.total.coin)
                elseif ldb_type == "none" then
                    AllPlayedLDB.text = "AllPlayed"
                else
...
-- Get the totals per faction and realm (with caching optimization)
function AllPlayed:ComputeTotal(force)
	 -- Перевіряємо кеш
	 if not force and self.totals_cache.valid and (time() - self.totals_cache.last_compute_time) < 5 then
	 	return
	 end
	...
	self.totals_cache.valid = true
	self.totals_cache.last_compute_time = time()
end

function AllPlayed:ComputeTotalHonor(force)
	-- Перевіряємо кеш для PvP даних
	if not force and self.totals_cache.honor_valid and (time() - self.totals_cache.honor_last_compute_time) < 10 then
		return
	end
	...
	self.total_faction["Horde"].arena_played     			= 0
	...
	self.total_faction["Alliance"].arena_played  			= 0
	...
	self.total_faction["Neutral"].arena_played   			= 0
	...
	self.total.arena_played                        				= 0
	self.total.arena_won                           				= 0
	self.total.arena_2v2_played                    				= 0
	...
	self.total.arena_solo_shuffle_won              				= 0
	 ...
	 for faction, faction_table in pairs(self.db.global.data) do
		  for realm, realm_table in pairs(faction_table) do
				...
				self.total_realm[faction][realm].arena_played = 0
				self.total_realm[faction][realm].arena_won = 0
				self.total_realm[faction][realm].arena_2v2_played = 0
				...
				for pc, pc_table in pairs(realm_table) do
					 if not self:GetOption('is_ignored', realm, pc) then
						...
						self.total_faction[faction].arena_played           = self.total_faction[faction].arena_played           + (pc_table.arena_played or 0)
						...
						self.total_realm[faction][realm].arena_played       = self.total_realm[faction][realm].arena_played       + (pc_table.arena_played or 0)
	 ...
	 if self:GetOption('all_realms') then
		  if self:GetOption('all_factions') then
				...
				self.total.arena_played
					 =   self.total_faction["Horde"].arena_played
						+ self.total_faction["Alliance"].arena_played
						+ self.total_faction["Neutral"].arena_played
				self.total.arena_won
					 =   self.total_faction["Horde"].arena_won
						+ self.total_faction["Alliance"].arena_won
						+ self.total_faction["Neutral"].arena_won
				self.total.arena_2v2_played
					 =   self.total_faction["Horde"].arena_2v2_played
						+ self.total_faction["Alliance"].arena_2v2_played
						+ self.total_faction["Neutral"].arena_2v2_played
				self.total.arena_2v2_won
					 =   self.total_faction["Horde"].arena_2v2_won
						+ self.total_faction["Alliance"].arena_2v2_won
						+ self.total_faction["Neutral"].arena_2v2_won
				self.total.arena_3v3_played
					 =   self.total_faction["Horde"].arena_3v3_played
						+ self.total_faction["Alliance"].arena_3v3_played
						+ self.total_faction["Neutral"].arena_3v3_played
				self.total.arena_3v3_won
					 =   self.total_faction["Horde"].arena_3v3_won
						+ self.total_faction["Alliance"].arena_3v3_won
						+ self.total_faction["Neutral"].arena_3v3_won
				self.total.arena_solo_shuffle_played
					 =   self.total_faction["Horde"].arena_solo_shuffle_played
						+ self.total_faction["Alliance"].arena_solo_shuffle_played
						+ self.total_faction["Neutral"].arena_solo_shuffle_played
				self.total.arena_solo_shuffle_won
					 =   self.total_faction["Horde"].arena_solo_shuffle_won
						+ self.total_faction["Alliance"].arena_solo_shuffle_won
						+ self.total_faction["Neutral"].arena_solo_shuffle_won
		  else
				-- Only the current faction count
				...
				self.total.arena_played 			= self.total_faction[self.faction].arena_played
				self.total.arena_won 				= self.total_faction[self.faction].arena_won
				self.total.arena_2v2_played 		= self.total_faction[self.faction].arena_2v2_played
				self.total.arena_2v2_won 			= self.total_faction[self.faction].arena_2v2_won
				self.total.arena_3v3_played 		= self.total_faction[self.faction].arena_3v3_played
				self.total.arena_3v3_won 			= self.total_faction[self.faction].arena_3v3_won
				self.total.arena_solo_shuffle_played 	= self.total_faction[self.faction].arena_solo_shuffle_played
				self.total.arena_solo_shuffle_won    	= self.total_faction[self.faction].arena_solo_shuffle_won
		  end
	 else
		  -- Only the current realm count (all_factions is ignore)
		  ...
		  self.total.arena_played  			= self.total_realm[self.faction][self.realm].arena_played
		  self.total.arena_won  				= self.total_realm[self.faction][self.realm].arena_won
		  self.total.arena_2v2_played  		= self.total_realm[self.faction][self.realm].arena_2v2_played
	...
	-- Позначаємо кеш як валідний
	self.totals_cache.honor_valid = true
	self.totals_cache.honor_last_compute_time = time()
end
...
function AllPlayed:PrintArenaSummary()
	...
	local pss  = self.total.arena_solo_shuffle_played or 0
	local wss  = self.total.arena_solo_shuffle_won or 0
	local played = p2v2 + p3v3 + pss
	local won    = w2v2 + w3v3 + wss
	...
	local pct_ss  = pss  > 0 and (wss  / pss  * 100) or 0
	...
	_G.DEFAULT_CHAT_FRAME:AddMessage(string.format(fmt, "Solo Shuffle: ", pss,  pct_ss))
			...
			for _, realm in ipairs(self.sort_faction_realm[self:GetOption('display_sort_type')][faction]) do
				...
				if ( (self:GetOption('all_realms') or self.realm == realm) and
					...
					if self:GetOption('show_arena_played') then
						...
						if first_option then
							text_realm_optional = arena_summary
							first_option = false
						else
							text_realm_optional = ("%s " .. C:Green(" : ") .. "%s"):format(text_realm_optional, arena_summary)
						end
					end
					...
					tooltip:SetLineColor(line, 0.1, 0.1, 0.1, 0.8)
...
if type(pc_data.last_update) ~= "number" then
	err("realm = %s, pc = %s, pc_data.last_update = %s", realm, pc, pc_data.last_update)
							...
							line, column = tooltip:AddLine()
							if self:GetOption('show_shading') then
								local color = (i % 2) == 1 and 0.15 or nil
								if color then tooltip:SetLineColor(line, color, color, color, 0.4) end
							end
	...
	if self:GetOption('show_arena_totals') and (self:GetOption('show_arena_played') or self:GetOption('show_arena_won')) then
		...
		local total_3v3w   = self.total.arena_3v3_won or 0
		local pct     = total_played > 0 and (total_won    / total_played * 100) or 0
		local pct_2v2 = total_2v2p  > 0 and (total_2v2w   / total_2v2p   * 100) or 0
		local pct_3v3 = total_3v3p  > 0 and (total_3v3w   / total_3v3p   * 100) or 0
		line, column = tooltip:AddLine()
		tooltip:SetCell(line, 1, C:Yellow( '2v2:' ), 'LEFT', nb_columns - 1)
		tooltip:SetCell(line, nb_columns, C:White(('%d'):format(total_2v2p)) .. C:Green((' (%.0f%%)'):format(pct_2v2)), 'RIGHT')
		line, column = tooltip:AddLine()
		tooltip:SetCell(line, 1, C:Yellow( '3v3:' ), 'LEFT', nb_columns - 1)
		tooltip:SetCell(line, nb_columns, C:White(('%d'):format(total_3v3p)) .. C:Green((' (%.0f%%)'):format(pct_3v3)), 'RIGHT')
		line, column = tooltip:AddLine()
		tooltip:SetCell(line, 1, C:Orange( L['Total Arenas: '] ), 'LEFT', nb_columns - 1)
		tooltip:SetCell(line, nb_columns, C:White(('%d'):format(total_played)) .. C:Green((' (%.0f%%)'):format(pct)), 'RIGHT')
	end
...
function AllPlayed:EventHandlerOnlySort(msg)
	 ...
	 -- Позначаємо, що таблиці сортування треба перебудувати
	self.sort_tables_done = false
end
...
function AllPlayed:EventHandlerWithSort(msg)
	 ...
	 -- Позначаємо, що таблиці сортування треба перебудувати і дані оновились
	self.sort_tables_done = false
...
function AllPlayed:EventHandler(msg)
	 ...
	 -- Інвалідуємо кеш та позначаємо що дані змінилися
	 self.totals_cache.valid = false
	 self.data_dirty = true
	 
	 -- Compute totals (використовуючи кеш)
	 self:ComputeTotal()
end
...
function AllPlayed:EventHandlerHonorGain()
	 ...
	 -- Інвалідуємо кеш PvP даних
	 self.totals_cache.honor_valid = false
	 self.data_dirty = true
...
local STAT_ARENAS_PLAYED         = 838
local STAT_ARENAS_WON            = 837
local STAT_2V2_PLAYED            = 367
local STAT_2V2_WON               = 366
local STAT_3V3_PLAYED            = 365
local STAT_3V3_WON               = 364
...
function AllPlayed:SaveVar()
	...
	pc.guild					= _G.GetGuildInfo("player") or ""
	 
	 -- If UnitXPMax returns 0 (can happen at high levels in some WoW versions), use cached value
	 if unit_xp_max == 0 and pc.level < self.max_pc_level then
	 	unit_xp_max = XPToNextLevel(pc.level)
	 end
	 
	 -- Only update max_rested_xp if we have a valid unit_xp_max value
	 -- This prevents max_rested_xp from being set to 0 when UnitXPMax() fails
	 if unit_xp_max > 0 or pc.level >= self.max_pc_level then
	 ...
	 elseif not pc.max_rested_xp or pc.max_rested_xp == 0 then
		 -- If we still don't have a valid max_rested_xp, use a safe fallback
		 -- Use the cached XP value with Inner Peace multiplier
		 local cached_xp = XPToNextLevel(pc.level) or 0
		 if cached_xp > 0 then
			 pc.max_rested_xp = cached_xp * 1.5 * (1 + (_G.IsSpellKnown(107074) and 1 or 0))
		 end
	 end
	 pc.last_update     		= time()
...
function AllPlayed:SetSecondsPlayed(seconds_played)
	...
	if not data[self.faction] then data[self.faction] = {} end
	if not data[self.faction][self.realm] then data[self.faction][self.realm] = {} end
	if not data[self.faction][self.realm][self.pc] then data[self.faction][self.realm][self.pc] = {} end
	local pc = data[self.faction][self.realm][self.pc]
...
function AllPlayed:SetOption( option, value, ... )
	...
	if option == 'all_factions' or option == 'all_realms' or option == 'is_ignored' or option == 'ldb_text_type' then
		if option == 'is_ignored' then
			local realm, name = ...
	...
	elseif option == 'show_seconds' then
		...
		if value then
			...
			-- If only the minutes are shown, we can refresh less often
...
-- was in an Inn
function AllPlayed:EstimateRestedXP( pc, realm, level, rested_xp, max_rested_xp, last_update, is_resting, current_xp )
	 ...
	 -- Защита від некоректного max_rested_xp (якщо 0 або nil)
	 if not max_rested_xp or max_rested_xp == 0 then
		  -- Спробувати отримати правильне значення з кешу
		  local cached_xp = XPToNextLevel(level)
		  if cached_xp and cached_xp > 0 then
			  max_rested_xp = cached_xp * 1.5
		  else
			  -- Якщо і кеш не допоміг, повернути збережене значення
			  return rested_xp
		  end
	 end
	 ...
	 if level == self.max_pc_level - 1 then
		  local xp_to_max = XPToNextLevelCache[self.max_pc_level - 1]
		  if xp_to_max and xp_to_max > 0 and current_xp then
			  estimated_rested_xp = min( estimated_rested_xp, xp_to_max - current_xp )
		  end
	 end
...
function AllPlayed:FormatTime(seconds)
	if not A or not A.FormatDurationFull then
...
function FormatCharacterName( pc, level, xp, seconds_played, class, class_loc, faction, last_update, is_resting )
	...
	local class_icon = ""
	if AllPlayed:GetOption('show_class_icon') and class and class ~= "" then
		class_icon = ("|TInterface\\Icons\\ClassIcon_%s:14|t "):format(class)
	end

	local resting_icon = ""
	if AllPlayed:GetOption('show_resting_indicator') and is_resting then
		resting_icon = " |TInterface\\CharacterFrame\\UI-State-RestingPlayerAnim:16:16:0:0:64:64:0:40:0:40|t"
	end

	result_string = class_icon .. ClassColour( class, faction, "%s (%s)" ):format( pc, level_string ) .. resting_icon
...
do
	...
	function PCSortByPercentRest(a,b)
		 if not a or not b or not table_to_sort[a] or not table_to_sort[b] then
		 ...
		 local max_rested_xp_a = (a and table_to_sort[a] and table_to_sort[a].max_rested_xp) or 0
		 local max_rested_xp_b = (b and table_to_sort[b] and table_to_sort[b].max_rested_xp) or 0

		 if max_rested_xp_a <= 0 and a and table_to_sort[a] then
			 local fallback_a = XPToNextLevel(table_to_sort[a].level)
			 if fallback_a and fallback_a > 0 then max_rested_xp_a = fallback_a * 1.5 end
		 end
		 if max_rested_xp_b <= 0 and b and table_to_sort[b] then
			 local fallback_b = XPToNextLevel(table_to_sort[b].level)
			 if fallback_b and fallback_b > 0 then max_rested_xp_b = fallback_b * 1.5 end
		 end

		 if max_rested_xp_a > 0 then percent_a = estimated_rested_xp_a / max_rested_xp_a end
		 if max_rested_xp_b > 0 then percent_b = estimated_rested_xp_b / max_rested_xp_b end

		 if percent_a ~= percent_b then
...
function InitXPToLevelCache( game_version, build_version )
	...
	XPToNextLevelCache[60]	= 254000
	XPToNextLevelCache[61]	= 275000
	XPToNextLevelCache[62]	= 301000
	XPToNextLevelCache[63]	= 328000
	XPToNextLevelCache[64]	= 359000
	XPToNextLevelCache[65]	= 367000
	XPToNextLevelCache[66]	= 374000
	XPToNextLevelCache[67]	= 381000
	XPToNextLevelCache[68]	= 388000
	XPToNextLevelCache[69]	= 395000
	XPToNextLevelCache[70]	= 405000
	XPToNextLevelCache[71]	= 415000
	...
	XPToNextLevelCache[83]	= 497000
	XPToNextLevelCache[84]	= 506000
	XPToNextLevelCache[85]	= 517000
	XPToNextLevelCache[86]	= 545000
	XPToNextLevelCache[87]	= 550000
	XPToNextLevelCache[88]	= 556000
	XPToNextLevelCache[89]	= 562000
	XPToNextLevelCache[90]	= 774800
	XPToNextLevelCache[91]	= 783900
	XPToNextLevelCache[92]	= 790400
	XPToNextLevelCache[93]	= 798200
	XPToNextLevelCache[94]	= 807300
...
function AllPlayedLDB:OnClick(button,down)
	...
	-- Не викликаємо UI, якщо відкритий Blizzard GameMenuFrame
	if GameMenuFrame and GameMenuFrame:IsShown() then return end
